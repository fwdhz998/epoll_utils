...
static void closesocket(int fd)
{
	shutdown(fd, SHUT_RDWR);//shutdown与socket描述符没有关系，即使调用shutdown(fd, SHUT_RDWR)也不会关闭fd，最终还需close(fd)。
	close(fd);
}
...
static void *accept_thread(void *arg)
{
	...
	val = 2; //2 minitues
	err = setsockopt(listen_fd, IPPROTO_TCP, TCP_DEFER_ACCEPT, &val, sizeof(val));//将允许一个监听者只在有数据到达这个套接字的时候才会被唤醒
	if (0 != err)
	{
...
//	set_non_blocking(listen_fd);
	...
	listen(listen_fd, LISTENQ); // 4096 TCP模块允许的"已完成"三次握手过程(TCP模块完成)但还没来得及被应用程序accept的最大链接数.
...
int main(int argc, char *argv[])
{
	...
	setsid();//执行setsid()之后,child将重新获得一个新的会话(session)id。
	...
	while (!exit_flag)
	{
		...
		for (index = 0; index < epoll_events_number; ++index) // deal with the event
		{
			...
			if (events[index].events & EPOLLIN) //have read event
			{
				...
				LOG_INFO(LOG_LEVEL_INDISPENSABLE, log_str_buf);
				// no the event
				if (event_index < 0)//This is not impossible,why happens this,as it must be in events[]
				{
					connect_total_count_opration(FALSE, 1);
