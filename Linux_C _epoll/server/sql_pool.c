	...
	do
	{
		...
		else
		{
			...
			else
			{
				...
				mysql_options(&node->fd, MYSQL_OPT_RECONNECT, &opt);//如果使用了长连接而长期没有对数据库进行任何操作，那么在 timeout值后，mysql server就会关闭此连接
				opt = 3; //3S
				mysql_options(&node->fd, MYSQL_OPT_CONNECT_TIMEOUT, &opt);//读写不能超过3s
				ret = 0;
			}
		}
